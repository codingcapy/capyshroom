...
export async function sendSecondEmail(req: Request, res: Response) {
    try {
        emails.forEach((email) => {
            return new Promise((resolve, reject) => {
                var transporter = nodemailer.createTransport({
                ...
                const mail_configs = {
                    from: '"Steph & Paul" <spkim0921@gmail.com>',
                    to: email.toString(),
                    subject: "👩‍❤️‍👨 Less than one month to go!!!",
                    html: `<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <title>👩‍❤️‍👨 Less than one month to go!!!</title>
</head>

<body
    style="margin: 0; padding: 0; font-family: Georgia, 'Times New Roman', Times, serif; letter-spacing:0.2px; line-height: 25px; text-align: left;">
    <div style="max-width: 500px; margin: 20px auto; background-color: #FFF7EE; padding: 16px; border-radius: 5000px;">
        <div style="margin: 0 auto; border: 2px solid #637CC6; padding: 6px; border-radius: 5000px;">
            <div
